...
- Added end-to-end RTL direction handling for Arabic and Persian across the
  ...
  engine.
- Added semantic `start`/`end` alignment, logical table ordering, semantic
  branding anchors, mixed-direction field isolation, and Arabic-capable font
...
- Mirrored application chrome with CSS logical properties while retaining
  physical LTR behavior for the page canvas, source editors, coordinates,
...
- Added a dedicated report preview/output guide covering filter transfer, secured snapshots, complete-result compilation, pagination, localization/RTL, bounded selectable PDF text, output audit boundaries, and large-report operations.
...
- Made Lilaq selection invariant for supported report charts; sanitized Frappe SVG is available only as a compatibility fallback for unsupported or invalid native specs.
- Added a dedicated report-chart guide with renderer architecture, fallback security policy, native limits and semantics, Branding Profile controls, diagnostics, and reproducible Lilaq-rendered illustrations for every supported chart kind.
...
- Expanded upstream report compatibility checks to composite HTML/JavaScript/JSON/Python fingerprints, installed-version metadata, missing/renamed/disabled/custom Report registry diagnostics, structural result snapshots, an explicit review checklist, and permission-checked fingerprint acknowledgement.
- Established the Report format lifecycle: one exact Report per published template history, current generator/fingerprint publication gates, centralized company/global default precedence, immutable minor/major template versions, and dedicated metadata-only report-output auditing for downloads without CID.
...
- Changed live Report preview execution to retain the complete prepared ERPNext result in a secured user/report/company/preview-tab-bound, 15-minute server snapshot. Builder-only column, width, filter-display, summary, total-row, typography, layout, and presentation changes project and recompile that snapshot instead of rerunning the report.
...
- Changed the Query Report handoff to preserve complete structured filter state, including arrays, accounting dimensions, optional blank values, checkboxes, dynamic columns, and route context. Presentation changes reuse the retained result; changing report inputs obtains a new snapshot.
...
- Changed the Basic report generator to signature version 4. It retains explicit native-chart section markers and `crispy-chart` calls while adding the managed pagination and language/RTL contracts. Existing Basic formats must be opened, saved, reviewed, and republished to adopt the current generated source; frozen templates are never rewritten.
- Changed report preview and PDF payloads to preserve the original ERPNext chart while returning the printable chart specification, selected engine, fallback/omission reason, helper version, and Lilaq version. Advanced formats receive `chart_svg` only when sanitized Frappe SVG is the explicitly resolved compatibility fallback.
...
- Fixed restored Query Report pages so the Crispy Print action rebinds to the current report instance instead of carrying stale filters or dynamic columns after browser navigation.
