...
export function buildDocDictionary(
...
class JSONTypstTranslator {
  ...
  private get_presentation_settings(): PresentationSettings {
    if (!this._presentation_settings) {
      ...
      this._presentation_settings = merge_presentation_settings(
        default_presentation_settings,
        overrides,
      );
    }
  ...
  private get_direction_context() {
    return getLanguageDirection(
      this.get_presentation_settings().language || "en",
    );
  }

  private resolve_field_alignment(
    alignment: LogicalAlignment | null | undefined,
    fieldtype: string,
  ): "left" | "center" | "right" {
    const requested = alignment || this.getDefaultAlignment(fieldtype);
    return resolveLogicalAlignment(
      requested,
      this.get_direction_context().direction,
    );
  }

  private render_field_label(label: string): string {
    const context = this.get_direction_context();
    const alignment = resolveLogicalAlignment("start", context.direction);
    return `#align(${alignment})[#text(dir: ${context.direction}, ..fieldLabelStyle)[${label}]]`;
  }

  private render_field_value(
    fieldname: string,
    fieldtype: string,
    alignment?: LogicalAlignment,
  ): string {
    const context = this.get_direction_context();
    const direction = getContentDirection(
      fieldtype,
      fieldname,
      context.direction,
    );
    const resolvedAlignment = this.resolve_field_alignment(
      alignment,
      fieldtype,
    );
    return `#align(${resolvedAlignment})[#text(dir: ${direction}, ..fieldValueStyle)[#doc.${fieldname}]]`;
  }
  ...
  generateUserSection() {
    ...
    const bodyFont = typography.fieldValue?.fontFamily || "Inter";
    const region = direction.region
      ? `, region: "${escapeTypstString(direction.region)}"`
      : "";
    lines.push(
      `#set text(font: ("${escapeTypstString(bodyFont)}", "Noto Naskh Arabic", "Noto Sans Arabic", "Inter"), lang: "${escapeTypstString(direction.languageCode)}"${region}, dir: ${direction.direction})`,
    );
    lines.push("");
    lines.push(
    ...
    lines.push(
      '#let cp_is_zero_tax_value(value) = value == 0 or value == "0" or value == "0.0" or value == "0.00" or value == ""',
    );
    lines.push("#let cp_is_zero_tax_row(row) = {");
    lines.push(
      '  if "tax_amount" in row { cp_is_zero_tax_value(row.tax_amount) }',
    );
    lines.push(
      '  else if "base_tax_amount" in row { cp_is_zero_tax_value(row.base_tax_amount) }',
    );
    lines.push(
      '  else if "amount" in row { cp_is_zero_tax_value(row.amount) }',
    );
    ...
    lines.push(
      '  else if "stock_uom" in row and row.stock_uom != "" { row.stock_uom }',
    );
    lines.push('  else { "" }');
    lines.push("}");
    ...
    lines.push("");
    const typstPreamble =
      (this.options.typstPreamble as string | undefined) || "";
  ...
  buildPageSetupBlock() {
    ...
    const branding_mode = resolveBrandingMode(
      presentation_settings,
      this.letterhead,
    );
    const letterheadFilename = getLetterheadFilename(
      presentation_settings,
      this.letterhead,
    );
    ...
    if (
      ...
      if ((this.letterhead as any).letter_head_name) {
        lines.push(
          `  // Letterhead: ${(this.letterhead as any).letter_head_name}`,
        );
      }
  ...
  generateAutoSection() {
    ...
    const printContext =
      (this.realDocData as any)?.__crispy_print_context || {};
    if (printContext.show_draft_heading) {
      lines.push(
        '#align(center)[#text(size: 18pt, weight: "bold", fill: rgb("#b91c1c"))[Draft]]',
      );
      lines.push("#v(1em)");
      lines.push("");
    }
  ...
  translateSection(
    ...
    const hasFields =
      section.columns?.some((col) => col.fields && col.fields.length > 0) ||
      false;
    if (!hasFields) {
    ...
    if (section.label) {
      const direction = this.get_direction_context();
      const alignment = resolveLogicalAlignment("start", direction.direction);
      lines.push(
        `#block(spacing: 0.6em)[#align(${alignment})[#text(dir: ${direction.direction}, ..sectionLabelStyle)[${safeLabel}]]]`,
      );
      lines.push("");
    }
    ...
    if (isRawTypstSection) {
      const maxRows = Math.max(
        ...section.columns.map((col) => col.fields?.length || 0),
      );
        ...
        for (let colIdx = 0; colIdx < numCols; colIdx++) {
          ...
          if (field) {
            if (field.fieldtype === "Table") {
              ...
              if (
                this.realDocData &&
                !(fieldname in (this.realDocData as Record<string, any>))
              ) {
              ...
              if (!field.table_columns || field.table_columns.length === 0) {
                lines.push(
                  `  // TODO: Table ${fieldname} has no columns defined`,
                );
              }
            }
  ...
  translateFieldAsCell(field: LayoutField): string {
    ...
    switch (fieldtype) {
      ...
      case "Crispy Typst Block": {
        ...
        return code
          ? this.formatContentCell(code)
          : `[] // Missing Crispy Typst Block: ${blockKey}`;
      }
      case "Spacer": {
      ...
      default:
        ...
        if (!label) {
          return `[${this.render_field_value(fieldname, fieldtype, field.align)}]`;
        }
        return `[${this.render_field_label(label)}#linebreak()${this.render_field_value(
          fieldname,
          fieldtype,
          field.align,
        )}]`;
  ...
  translateField(field?: LayoutField) {
    ...
    switch (fieldtype) {
      ...
      default:
        ...
        if (!label) {
          return `${this.render_field_value(fieldname, fieldtype, field.align)}#parbreak()`;
        }
        return `${this.render_field_label(label)}#linebreak()${this.render_field_value(
          fieldname,
          fieldtype,
  ...
  getDefaultAlignment(fieldtype: string): LogicalAlignment {
  ...
  private isCrispyImageField(field: LayoutField): boolean {
    return (
      field.fieldtype === "Crispy Image" || field.fieldname === "_crispy_image"
    );
  }

  private renderCrispyImage(
    field: LayoutField,
  ...
  translateTable(
    ...
    if (field.table_columns && field.table_columns.length > 0) {
      ...
      const compactItems =
        this.is_enabled_behavior("compact_item_print") && itemTable;
      ...
      const alignments = effectiveColumns.flatMap((col) => {
        const align = this.resolve_field_alignment(col.align, col.fieldtype);
        return this.shouldSplitTableCellLabel(col, {
          itemTable,
          splitCellLabels,
        })
          ? [align, align]
      ...
      const headerCells = effectiveColumns
        .map((col) => {
          ...
          const headerAlign = resolveLogicalAlignment(
            "start",
            direction.direction,
          );
          const header = `#align(${headerAlign})[#text(dir: ${direction.direction}, ..tableHeaderStyle)[${label}]]`;
          return this.shouldSplitTableCellLabel(col, {
            itemTable,
            splitCellLabels,
          })
  ...
  private is_item_table(fieldname: string): boolean {
    return ["items", "packed_items"].includes(
      String(fieldname || "").toLowerCase(),
    );
  }

  private is_tax_table(fieldname: string): boolean {
    return ["taxes", "taxes_and_charges"].includes(
      String(fieldname || "").toLowerCase(),
    );
  }
  ...
  private renderTableCell(
    ...
    if (
      ...
      return `[#text(dir: ltr, ..tableBodyStyle)[#cp_quantity_inline(row, row.${fieldname})]]`;
    }
    if (options.itemTable && col.fieldtype === "Currency") {
      ...
      return `[#text(dir: ltr, ..tableBodyStyle)[#row.${fieldname}]]`;
    }
    return `[#text(dir: ${direction}, ..tableBodyStyle)[#row.${fieldname}]]`;
  }
  ...
  private getCompactItemColumns(columns: TableColumn[]): {
    ...
    const visible = columns.filter((col) =>
      compactFieldnames.has(col.fieldname || ""),
    );
    const folded = columns.filter(
      (col) => !compactFieldnames.has(col.fieldname || ""),
    );

    if (
      folded.length &&
      !visible.some((col) => col.fieldname === "description")
    ) {
      visible.unshift({
        fieldname: "description",
        label: "Description",
