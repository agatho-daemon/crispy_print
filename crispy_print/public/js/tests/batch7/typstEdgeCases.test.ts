...
describe("Typst edge cases", () => {
            ...
            columns: [
              {
                label: "",
                fields: [
                  { fieldname: "intro", fieldtype: "Data", label: "Intro" },
                ],
              },
            ],
            page_break: true,
  ...
  it("honors print behavior options for item quantity and zero tax rows", () => {
    ...
    expect(typst).toContain(
      "table.cell(colspan: 2)[#align(left)[#text(dir: ltr, ..tableHeaderStyle)[Qty]]]",
    );
    expect(typst).toContain(
      "cp_measure_label_table_cell(cp_quantity_parts(row, row.qty).label)",
    );
    expect(typst).toContain(
      "cp_measure_value_table_cell(cp_quantity_parts(row, row.qty).value)",
    );
    expect(typst).toContain("#let cp_print_uom_after_quantity = false");
    expect(typst).toContain("baseline: -2pt");
    expect(typst).toContain(
      "cp_measure_label_table_cell(cp_currency_parts(row.rate).label)",
    );
    expect(typst).toContain(
      "cp_measure_value_table_cell(cp_currency_parts(row.rate).value)",
    );
    expect(typst).toContain("cp_currency_parts(value)");
    expect(typst).toContain(
      "doc.taxes.filter(row => not cp_is_zero_tax_row(row))",
    );
  });
  ...
  it("can disable split table cell labels while preserving custom label style constants", () => {
    ...
    expect(typst).not.toContain(
      "table.cell(colspan: 2)[#text(..tableHeaderStyle)[Qty]]",
    );
    expect(typst).not.toContain(
      "cp_measure_label_table_cell(cp_quantity_parts(row, row.qty).label)",
    );
    expect(typst).toContain(
      "[#text(dir: ltr, ..tableBodyStyle)[#cp_quantity_inline(row, row.qty)]]",
    );
    expect(typst).toContain('if cp_print_uom_after_quantity and uom != ""');
    expect(typst).toContain('uom + " " + str(value)');
    expect(typst).toContain("[#text(dir: ltr, ..tableBodyStyle)[#row.rate]]");
  });
