import type { TableSettings } from "./pageSettings"
...
export function normalizeReportBuilderConfig(
	...
	return {
		...
		font_size_pt: toNumber(raw.font_size_pt, defaults.font_size_pt),
...
export function buildReportTypstFromConfig(
	...
	lines.push(`#set text(font: "${config.font_family}", size: ${formatPt(fontSize)})`)
	lines.push("")
	...
	lines.push("#table(")
	lines.push("  columns: data.columns.map(col => {")
	lines.push('    if col.width == "auto" {')
	lines.push("      auto")
	lines.push("    } else {")
	lines.push("      eval(col.width)")
	lines.push("    }")
	lines.push("  }),")
	lines.push("")
	...
	lines.push(
		`    ..data.columns.map(col => text(font: "${headerFontFamily}", size: ${headerFontSize}, style: "${headerFontStyle}", weight: "${headerFontWeight}", fill: rgb("${headerFontColor}"))[#col.label])`
	)
	lines.push("  ),")
	lines.push("")
	lines.push("  ..data")
