...
- **Frappe Print Engine Adapter** - Registers Crispy Print through Frappe's `print_engines` hook so Frappe's Print button can hand document printing directly to the Typst preview route when an approved active Crispy Template resolves.
...
- **Frappe Print Engine integration:** Crispy Print registers a `crispy_print` client renderer through Frappe's hook-based print engine dispatch. Sites keep Frappe's native print flow until an administrator enables **Use Crispy Print as Default Print Engine** in Crispy Print Settings.
...
Frappe's native **Print** action can also open the same Crispy preview route after an administrator enables **Use Crispy Print as Default Print Engine** in Crispy Print Settings. If no active approved template resolves for the document, the adapter falls back to Frappe's native print view. The existing **Typst** button still works.
